...
class Investigador extends Sistema {
    ...
    try {
        ...
        $sql = "INSERT INTO usuario (correo, contrasenia)
                VALUES (:correo, :contrasenia)";
        $sth = $this->_DB->prepare($sql);
        $sth->bindParam(":correo", $data['correo'], PDO::PARAM_STR);
        $pwd = md5($data['contrasenia']);
        $sth->bindParam(":contrasenia", $pwd, PDO::PARAM_STR);
        $sth->execute();
        ...
        $sql = "INSERT INTO usuario_role (id_rol, id_usuario)
                VALUES (2, :id_usuario)";
        $sth = $this->_DB->prepare($sql);
